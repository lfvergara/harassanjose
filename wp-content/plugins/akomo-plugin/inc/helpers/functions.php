...
function get_hb_room_categories() {
	$options = array();
...
function akomo_the_pagination2($args = array(), $echo = 1)
{
...
add_filter('akomo_redux_custom_fonts_load', 'akomo_redux_custom_fonts_load');
function akomo_redux_custom_fonts_load( $custom_font ) {
	$custom_style = '';
	...
	if ( $filename = akomo_set( $pathinfo, 'filename' ) ) {
		...
		foreach ( $extensions as $extension ) {
			$file_path = esc_url(home_url('/')) . '/wp-content/themes/akomo/assets/css/custom-fonts/' . $filename . '.' . $extension;
...
/**
 * [akomo_social_share_output description]
 *
 ...
 */
function akomo_social_share_output( $icon, $color = false ) {
...
function akomo_get_social_icons()
{
	...
	if( akomo_set( $options, 'social_media' ) && is_array( akomo_set( $options, 'social_media' ) ) )
	{
		$total = count( akomo_set( $options, 'social_media' ) ) - 2;
		foreach( akomo_set( $options, 'social_media' ) as $social_icon ){
			if( isset( $social_icon['tocopy' ] ) ) continue;
			$title = akomo_set( $social_icon, 'title');
			$class = akomo_set( $social_icon, 'class');
			$link = akomo_set( $social_icon, 'social_link');
			$icon = akomo_set( $social_icon, 'social_icon');
...
add_action('akomo_social_share_output', 'akomo_social_share_output');
...
add_action('akomo_social_share_output_two', 'akomo_social_share_output_two');
...
<ul class="social-links">
	...
	<?php if($options->get( 'facebook_sharing' )):?>
    <li><a class="facebook" href="http://www.facebook.com/sharer.php?u=<?php echo esc_url(get_permalink($PostID)); ?>" target="_blank"><i class="fab fa-facebook-f"></i></a></li>
    <?php endif;?>

    <?php if($options->get( 'twitter_sharing' )):?>
    <li><a class="twitter" href="https://twitter.com/share?url=<?php echo esc_url(get_permalink($PostID)); ?>&text=<?php echo esc_attr($post_slug=$PostName); ?>" target="_blank"><i class="fab fa-twitter"></i></a></li>
    <?php endif;?>

    <?php if($options->get( 'linkedin_sharing' )):?>
    <li><a class="linkedin" href="http://www.linkedin.com/shareArticle?url=<?php echo esc_url(get_permalink($PostID)); ?>&title=<?php echo esc_attr($post_slug=$PostName); ?>"><i class="fab fa-linkedin"></i></a></li>
    <?php endif;?>

    <?php if($options->get( 'pinterest_sharing' )):?>
    <li><a class="pinterest" href="https://pinterest.com/pin/create/bookmarklet/?url=<?php echo esc_url(get_permalink($PostID)); ?>&description=<?php echo esc_attr($post_slug=$PostName); ?>"><i class="fab fa-pinterest"></i></a></li>
    <?php endif;?>

    <?php if($options->get( 'reddit_sharing' )):?>
    <li><a class="facebook" href="http://reddit.com/submit?url=<?php echo esc_url(get_permalink($PostID)); ?>&title=<?php echo esc_attr($post_slug=$PostName); ?>"><i class="fab fa-reddit"></i></a></li>
    <?php endif;?>

    <?php if($options->get( 'tumblr_sharing' )):?>
    <li><a class="twitter" href="http://www.tumblr.com/share/link?url=<?php echo esc_url(get_permalink($PostID)); ?>&name=<?php echo esc_attr($post_slug=$PostName); ?>"><i class="fab fa-tumblr"></i></a></li>
    <?php endif;?>

    <?php if($options->get( 'digg_sharing' )):?>
    <li><a class="linkedin" href="http://digg.com/submit?url=<?php echo esc_url(get_permalink($PostID)); ?>&title=<?php echo esc_attr($post_slug=$PostName); ?>"><i class="fab fa-digg"></i></a></li>
    <?php endif;?>
...
function akomo_get_social_icons2()
{
