...
class Header_Enqueue {
	...
	public static function scripts() {
		$options = get_theme_mod( 'akomo' . '_options-mods' );
		$ssl     = is_ssl() ? 'https' : 'http';

		$scripts = array(
		);

		$scripts = apply_filters( 'AKOMO/includes/classes/header_enqueue/scripts', $scripts );
		/**
		 * Enqueue the scripts
		 *
		 * @var array
		...
		$header_data = array(
			...
			'nonce'   => wp_create_nonce( AKOMO_NONCE ),
		);

		wp_localize_script( 'jquery', 'akomo_data', $header_data );

		if ( akomo_set( $options, 'footer_js' ) ) {

			wp_add_inline_script( 'jquery', akomo_set( $options, 'footer_js' ) );
		}
	...
	public static function styles() {
		...
		$styles = apply_filters( 'AKOMO/includes/classes/header_enqueue/styles', $styles );
		...
		foreach ( $styles as $name => $style ) {
			...
			if ( strstr( $style, 'http' ) || strstr( $style, 'https' ) || strstr( $style, 'fonts.googleapis' ) ) {
				wp_enqueue_style( "akomo-{$name}", $style );
			} else {
				wp_enqueue_style( "akomo-{$name}", get_template_directory_uri() . '/' . $style );
			}
		}
		$options      = akomo_WSH()->option();
		$custom_style = '';
		...
		if ( $custom_font = $options->get('theme_custom_font') ) {
            $header_styles .= akomo_custom_fonts_load( $custom_font );
        }

		wp_add_inline_style( 'akomo-main-style', $header_styles );
	}
	...
	public static function resource_hints( $urls, $relation_type ) {
		if ( wp_style_is( 'akomo-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
			$urls[] = array(
	...
	public static function header_styles() {
		...
		$data = \AKOMO\Includes\Classes\Common::instance()->data( 'blog' )->get();
		...
		$settings = get_theme_mod( 'akomo' . '_options-mods' );

		if ( $custom_font = akomo_set( $settings, 'theme_custom_font' ) ) {

			$styles .= apply_filters('akomo_redux_custom_fonts_load', $custom_font );
