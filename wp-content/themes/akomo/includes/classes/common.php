...
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				'layout'            => akomo_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => akomo_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => akomo_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => akomo_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => akomo_meta( 'banner_source_type', $page_id ),
				'banner_elementor'  => akomo_meta( 'banner_elementor_template', $page_id ),
				'banner'            => akomo_set( akomo_meta( 'banner_page_background', $page_id ), 'url' ),
				'title'             => akomo_meta( 'banner_banner_title', $page_id ),
				'enable_banner'     => akomo_meta( 'banner_page_banner', $page_id ),
			];
		} else {
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : akomo_the_title( $template ),
				'banner'        => akomo_set( $bg, 'url' ),
