 ...
 */
function akomo_unset_default_widgets_args( $widget_areas ) {
...
add_filter( 'merlin_unset_default_widgets_args', 'akomo_unset_default_widgets_args' );
 ...
 */
function akomo_child_functions_php( $output, $slug ) {
...
add_filter( 'merlin_generate_child_functions_php', 'akomo_child_functions_php', 10, 2 );
 ...
 */
function akomo_local_import_files() {
	return array(
		array(
			'import_file_name'         => esc_html__('Main Demo', 'akomo'),
			...
			'local_import_redux'       => array(
				array(
					...
					'option_name' => 'akomo_options',
				),
			),
			'local_import_file'        => trailingslashit( get_template_directory() ) . 'demo-import/content/content.xml',
			'import_preview_image_url' => get_template_directory_uri() . '/screenshot.png',
			'import_notice'            => esc_html__( 'After you import this demo, you will have to setup the slider separately.', 'akomo' ),
			'preview_url'              => 'http://commonsupport.net/newwp/akomo/',
		),
	);
}

add_filter( 'merlin_import_files', 'akomo_local_import_files' );

/**
 * Execute custom code after the whole import has finished.
 */
function akomo_after_import_setup() {
    ...
    if( class_exists('RevSliderSliderImport') ) {
		foreach(array('Home-V1') as $slider) {
...
add_action( 'merlin_after_all_import', 'akomo_after_import_setup' );